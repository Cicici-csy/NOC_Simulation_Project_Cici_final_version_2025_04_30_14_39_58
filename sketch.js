...
function preload() {
  ...
  StartImg = loadImage("startbackground.png");
...
function start() {
  ...
  while (fishes.length < 5) {
    fishes.push(new Fish(fishImg, 4, 25));
  }
  while (fishes2.length < 5) {
    fishes2.push(new Fish(fishImg2, 1, 15));
  }
...
function draw() {
  ...
  if (state === "start") {
    ...
    // text("Click to Dive into, a Dreamy Ocean World", width / 2, height / 2 + 50);
    if (growing) {
      buttonScale += 0.002;
    ...
    push();
    scale(buttonScale);
    image(playButton, windowWidth / 2 - 125, windowHeight / 2 - 110, 250, 125);
    pop();
    return;
    ...
    if (m <= 0) {
      ...
      image(finalImg,0,0,windowWidth,windowHeight);
    }
